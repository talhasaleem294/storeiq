...
import { BANK_DETAILS, PLAN_PRICES, ROUTES, TRIAL_DAYS, WHATSAPP_NUMBER } from '@/lib/constants'
...
export function Workspaces(): JSX.Element {
  ...
  // Trial workspace with the fewest days remaining (shown in countdown)
  const trialWorkspace = workspaces.find((ws) => ws.subscription_status === 'trial') ?? null
  const trialDaysLeft = trialWorkspace ? trialDaysRemaining(trialWorkspace.trial_started_at) : 0
  const showTrialBanner = !loading && trialWorkspace !== null
  const showActivationBanner =
    showBanner &&
    !showTrialBanner &&
    (workspaces.some((ws) => ws.subscription_status === 'inactive') ||
      (workspaces.length === 0 && pendingPlan !== null))
  ...
  return (
    <div className="min-h-svh bg-bg">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        ...
        {showTrialBanner && (
          <div className={`mb-6 rounded-xl border p-5 ${
            ...
            <div className="mb-3 flex items-center gap-2">
              <span className="text-lg">{trialDaysLeft > 0 ? '⏱️' : '⚠️'}</span>
              <h2 className={`text-sm font-semibold ${
                ...
                {trialDaysLeft > 0
                  ? `${String(trialDaysLeft)} day${trialDaysLeft === 1 ? '' : 's'} left in your trial`
                  : 'Trial expired'}
              </h2>
            </div>
            <p className={`mb-4 text-sm ${
              ...
              {trialDaysLeft > 0
                ? 'Transfer payment and WhatsApp your receipt to activate your account before the trial ends.'
                : 'Your trial has ended. Transfer payment to restore access to your dashboard.'}
            </p>

            <div className="mb-4 space-y-1 rounded-lg border border-current/10 bg-white/60 px-4 py-3 text-sm dark:bg-black/20">
              ...
              <div className="flex justify-between gap-4">
                <span className="text-text">Account No.</span>
                <span className="font-medium text-heading">{BANK_DETAILS.accountNumber}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span className="text-text">IBAN</span>
                <span className="font-medium text-heading">{BANK_DETAILS.iban}</span>
              </div>
            </div>
        ...
        {/* Pending activation banner (inactive workspaces / new signup with pending plan) */}
