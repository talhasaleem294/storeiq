    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <ProfitSummaryCard
          ...
          value={formatCurrency(totalInfluencerSpend)}
          loading={loading}
        />
        <ProfitSummaryCard
          label="Meta Ad Spend"
          value={formatCurrency(totalMetaSpend)}
          loading={loading}
        />
        <ProfitSummaryCard
          label="Combined Spend"
          value={formatCurrency(combinedSpend)}
          loading={loading}
          highlight
        />
      </div>
            ...
            {(deals).map(deal => (
              <Card key={deal.id} padding="md">
                <div className="flex justify-between text-xs">
                  <span className="font-medium text-heading">{formatDate(deal.deal_date)}</span>
                  <span className="font-semibold text-accent">{formatCurrency(deal.total_amount + deal.product_value)}</span>
                </div>
